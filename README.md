...
- **macOS 15.6 or newer** and **Xcode Command Line Tools** (`xcode-select --install`). The app compiles from source in a few seconds and is ad-hoc signed, so there is no Gatekeeper or notarization dance. The floor is set by the build, not the app: the mini player uses macOS 26 Liquid Glass APIs behind availability checks, so compiling needs the macOS 26 SDK, which comes with the Xcode 26 command-line tools, and Apple ships those for macOS 15.6 and up only. If Apple's installer gave you the 27.0 tools instead, they cannot compile SwiftUI by themselves (a compiler plugin ships only inside Xcode), so the build uses the macOS 26 SDK they still include; INSTALL.md checks that it is there.
