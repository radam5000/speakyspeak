...
**A macOS menu-bar app that reads Claude Code's replies aloud.** When Claude finishes a turn, a Stop hook renders the reply with a local neural voice and queues it. Replies play **one at a time**, so several parallel Claude sessions never talk over each other.
...
- **macOS 15.6 or newer** and **Xcode Command Line Tools** (`xcode-select --install`). The app compiles from source in a few seconds and is ad-hoc signed, so there is no Gatekeeper or notarization dance. The floor is set by the build, not the app: the mini player uses macOS 26 Liquid Glass APIs behind availability checks, so compiling needs the macOS 26 SDK or newer, and Apple ships those tools for macOS 15.6 and up only. The build picks its own SDK: it uses your default one, and falls back only if that cannot compile SwiftUI, which is the case for the 27.0 command-line tools on their own (a compiler plugin ships only inside Xcode) — those still carry the macOS 26 SDK, which works. INSTALL.md checks for it.
- **Claude Code**, installed and signed in — in a terminal, the desktop app, VS Code, or Cursor. Hooks and settings live in `~/.claude/` regardless of which one you use, so the install is the same. (It's the Claude *Code* Stop hook this hangs off; it does not read the Claude chat app aloud.) Not Claude Code on the web, and not a session running on another machine over SSH: the hook has to run on the Mac that plays the sound. Tested by hand in the terminal and the desktop app; VS Code and Cursor run the same hook mechanism.
- **Apple Silicon** for the neural voice (Kokoro via MLX). On Intel it falls back to the built-in macOS `say` voice and everything else works. The neural voice is English only for now: replies in another language come out as English sounds. If you work with Claude in another language, use the `say` engine (`~/.claude/speak-engine` = `say`) with a matching macOS voice in `~/.claude/speak-voice`.
...
- `~/.claude/hooks/speak-reply.sh` (Stop hook) — wakes the app, renders each reply with Kokoro (falls back to `say`), and drops `<epoch>-<sid>.m4a` + `.json` into the queue. The same script on the `Notification` hook makes **permission prompts audible**: when Claude Code stops to ask "may I run this?", it plays a distinct chime (Hero; override with a path in `~/.claude/speak-prompt-chime`) and speaks "Waiting on you in <session>. <the question>" through the deck, so a session behind another window never waits in silence. `~/.claude/hooks/session-end.sh` (SessionEnd hook) drops `.end` so the app clears that session's *played* items (unheard replies stay). Live copies in `~/.claude/hooks/`, sources in `hooks/` here — edit here, `./install.sh` copies. `install.sh` registers all four hook events (Stop, PostToolUse, Notification, SessionEnd) in `~/.claude/settings.json` itself: add-only, backup first, no-op when they are already there, so an in-app update can bring a new hook event to an existing install.
...
- **Skip = stop this reply.** The skip button (the now-playing card and the mini player) stops whatever's speaking and plays the next queued reply, or goes quiet if nothing's queued — unlike Mute/Pause, it doesn't shut the whole pipeline down, it just drops the one track you don't want to hear. Works even with an empty queue.
- **Mini player** — whenever a reply starts speaking, it drops under the menu-bar icon with the title and a full transport: **back-track / back-10 / play-pause / forward-10 / skip** (the two track buttons pinned to the far edges), so one click controls what you're hearing without opening the full deck. "Back-track" (far left) restarts the current reply, then steps to the previously-played one if you're already near the start (music-player style); "skip" (far right) stops the current one and moves on. A **glowing progress bar along the bottom** shows how far into the reply you are. It bridges the gap between queued replies and fades out a moment after things go quiet. **Drag it anywhere** — grab the background and move it; the spot sticks across launches instead of snapping back under the icon. When a reply starts — or you skip forward / step back / restart — it **flashes bright glowing white** for a moment so it's easy to spot wherever you've parked it. Its surface comes in three looks, set in Settings ▸ Appearance: **Classic** (opaque, reads the same over any wallpaper), **Glassy** (the default on macOS 26+) and **Glassier** (more transparent still). An **X dismisses it for the current reply only** — it pops back up on its own when the next reply starts. Non-activating, so it never steals focus or interrupts typing. Left-clicking the icon still opens the full deck.
...
- **Rows** — the row itself is inert; the left icon is pure status (bright orange dot = plays next, dim orange dot = queued, spark = speaking now, gray check = played). Actions are on the right: hover for **play-now / mute-session / remove**. Queued rows sit under a **session header** (title, reply count, time span) with its own hover actions: **Play this session next** and **Remove this session's queued replies**.
- **Muting a session** — the speaker-slash on any row silences that whole terminal session: its replies keep arriving and stay in the list, they just never play (if it's the one speaking, it stops immediately, like a skip). Every unplayed row of a muted session keeps a visible speaker-slash and a gray dot, so it's always obvious what's muted — click the icon again on any of its rows to unmute, and the backlog is playable again. Nothing is deleted; playback simply steps over muted sessions, and the menu-bar badge counts only replies that will actually play. A mute clears itself when the session ends (its terminal closes); clearing the list by hand doesn't unmute anything.
...
- **Settings** — the gear on the "Up next" line, or right-click the icon → Settings…. A native grouped window for the knobs that used to require editing dotfiles by hand (engine + voice), plus appearance, plus **About** (version and updates) and **Report an issue** (the two ways to send a report). See below.
- **Updates** — when a new version is out, the menu-bar icon grows a ↑ next to it and the full deck's footer shows "Update X available"; both point at **Settings ▸ About**, where the "Update to X…" button pulls, rebuilds, and relaunches. The same section has a "Check for updates" button when you're already current (it also checks once a day on its own).
- **Quit** — right-click the menu-bar icon → Quit (also right-click anywhere on the full deck itself).
- `~/.claude/speak-off` remains a shell-side hard kill (hook won't render at all); the app watches it and shows "Speech is off" when present.
- **AirPods / media keys** — the app publishes to macOS Now Playing while a reply is speaking (or paused mid-reply, or more are queued): one AirPods stem click pauses/resumes, double-click skips to the next reply, triple-click restarts/steps back; Control Center gets a working scrubber. The claim is released when the app goes idle, so headphone clicks return to Music/Spotify.
- **Shared-AirPods lead-in** — the headphones only switch to this Mac once it starts producing audio, and that takes about a second, which used to eat the opening words of the first reply after the other Mac spoke. When the app has been silent longer than 6s, playback is *scheduled* `speak-lead-in` seconds ahead (default 1.2) instead of started immediately: the output device engages now and runs through the gap, and the speech starts once the route has arrived. Chained replies never pay it (their gap is ~2s). `~/.claude/speak-lead-in` tunes it; `0` disables.
- **Two Macs, one pair of AirPods** — put the *other* Mac's Tailscale IP in `~/.claude/speak-peer` on each machine and the two Macs take turns: before *auto*-playing, each asks the peer (TCP 48765, "playing"/"idle") and holds until it's quiet, so macOS's AirPods auto-switching hands off at the gap instead of tug-of-warring mid-sentence. Manual plays never wait. Checks fail open — no file, peer off, or unreachable all mean "just play."
...
- **Read replies** — *when* speech happens. **When Claude finishes** (the default) reads a reply only once Claude stops and is waiting for you — the original behaviour. The other two read a long agentic run **as it happens**, which matters when Claude asks you to do something ("press the button now") twenty minutes into a run that hasn't ended yet: **As it works, skipping short lines** reads each step but stays quiet for short connecting lines like "Let me check that." (threshold: `~/.claude/speak-min-words`, default 15), and **As it works, every line** reads all of them. → `~/.claude/speak-when` (`end` | `substantial` | `all`; absent = `end`). `install.sh` registers the `PostToolUse` hook it needs (INSTALL.md step 5 checks it). It is the same script, and in the default mode it exits in ~30ms without doing anything.
- **Say the session name first** — every reply opens with its session name (the `/rename` name, else the folder), so with several sessions speaking in a row you know which project each is about. → `~/.claude/speak-name-first` (exists = on).
...
- **Accent colour** — one slider repaints the app's orange everywhere, including the menu-bar badge. Reset returns the original terracotta.
- **Appearance** — the **mini player** style — **Classic** (an opaque card, legible over any wallpaper), **Glassy** (Apple's adaptive `.regular` Liquid Glass, the default on macOS 26) or **Glassier** (`.clear` glass, more transparent, carrying the dimming layer Apple's guidance says `.clear` requires); both glass looks grey out below macOS 26; and **when it shows** — **Only while speaking** (auto-appears with a reply, fades after — the default) or **Always visible** (a persistent little controller that stays on screen; the X dismiss is hidden in this mode). All app prefs.
...
Sorting the list **is** sorting the queue: the app reads it from the top down.
...
- The now-playing card and the mini player show **"2 of 6"** — where this reply sits in its group's run, so you know how much of the story is left. It's hidden when nothing is grouped.
