...
- **Updates** — when a new version is out, the menu-bar icon grows a ↑ next to it and the deck's footer shows "Update X available"; both point at **Settings ▸ About**, where the "Update to X…" button pulls, rebuilds, and relaunches. The same section has a "Check for updates" button when you're already current (it also checks once a day on its own).
