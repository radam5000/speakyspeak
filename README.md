...
- `~/.claude/hooks/speak-reply.sh` (Stop hook) — wakes the app, renders each reply with Kokoro (falls back to `say`), and drops `<epoch>-<sid>.m4a` + `.json` into the queue. `~/.claude/hooks/session-end.sh` (SessionEnd hook) drops `.end` so the app clears that session's *played* items (unheard replies stay). Live copies in `~/.claude/hooks/`, sources in `hooks/` here — edit here, `./install.sh` copies. Wired in `~/.claude/settings.json` (snippet printed by install.sh).
